...
2. Активируйте виртуальное окружение:
    ...
    - macOS/Linux:
      ...
      ```
3. В файле settings.py (./testEffMob/settings.py) на строке 105 замените 
``*'HOST': 'db'*`` на ``*'HOST': 'loacalhost'*``
...
Система поддерживает три вида пользовательских ролей:
...
#### Примеры аккаунтов, которые уже существуют в системе (если вы полностью выполнили [п.1.1.](#11-запуск-с-использованием-docker) или [п.1.2](#12-установка-зависимостей-если-не-используете-docker)):
...
> Примечание: Пользователь с ролью "Администратор" может изменять роли всех пользователей в системе.
---
...
1. **Авторизация** (она же - главная страница):
   - Страница, через которую пользователь может войти в систему или зарегистрироваться.
   - Доступна всем пользователям.

2. **Профиль**:
   - Доступна только авторизованным пользователям для просмотра личной информации.
   - На этой странице доступны кнопки для перехода на другие страницы (например, редактирование профиля, выход из системы и другие).

3. **Страница администратора**:
   - Страница доступна только авторизованным пользователям с ролью **"Администратор"**.
   - На данной странице администратор может менять роли пользователей системы с использованием внутреннего API (см. п. [4.5](#45-два-api-внешний-и-внутренний)).
...
- **`user_id`**: Внешний ключ на модель **`Users`** (пользователь).
- **`role_id`**: Внешний ключ на модель **`Roles`** (роль).
...
Модель **`Passwords`** хранит пароли пользователей. Это позволяет отслеживать старые пароли, а также предотвратить их повторное использование (исторические данные).

#### Поля:
- **`date`**: Дата и время сохранения пароля (автоматически устанавливается).
- **`user_id`**: Внешний ключ на модель **`Users`** (пользователь).
- **`salt`**: Соль для хэширования пароля (необходимо для сверки "нового" и "старого" пароля в дальнейшем - см. [TODO](#2-работа-над-пользовательскими-паролями)). 
...
- **`user_id`**: Внешний ключ на модель **`Users`** (пользователь).
- **`action`**: Описание действия, которое выполнил пользователь (например, попытка аутентификации, авторизация, выход).
...
- Введенный пароль объединяется с солью и сравнивается с хэшем из базы данных при помощи функции check_password().
...
- добавить дополнительные методы управления пользователем, например:
...
- update_password(user, password) - используется при смене пароля пользователем (создает новый хэш и соль через вызов `hash_password()`, обновляет поле `password` у пользователя в таблице `Users` и сохраняет запись в таблице `Passwords`).
...
Предназначен для взаимодействия с системой извне — например, через Postman, curl, Python-скрипты ([пример кода](#23-внешний-функционал-системы)) или другие интеграции. Он защищен JWT-аутентификацией и предназначен для безопасного обмена данными с внешними клиентами. Особенности:
...
Система разработана с учетом будущего расширения - для релализации других сущностей системы (например, **деревья файлов** или **узлы деревьев**). Для каждой из этих сущностей будет определен свой **`ACCESS_LEVEL`**.
...
- Каждая сущность будет иметь свой заранее определенный (заданный, как константа) **`ACCESS_LEVEL`**, который будет сравниваться с ролью пользователя перед предоставлением доступа.
...
- При восстановлении доступа к аккаунту пользователь должен ввести хотя бы один из старых паролей (или тот, который он помнит) для дополнительной проверки.
...
Для удобства управления системой и обеспечения первого доступа к функционалу администрирования необходимо реализовать возможность автоматического создания **пользователя с ролью "Администратор"** при запуске системы (сейчас это реализовано автоматически через подгрузку данных в БД при первом старте системы). Реализация данного функционала возможна, например, через добавление вызова метода во views.py:
